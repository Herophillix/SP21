...
void renderGame()
{
	if (g_eGameState == S_GAME)
	{
		...
		else if ((g_eGamemode == S_STAGEONE) || (g_eGamemode == S_STAGETHREE))
		{
...
void renderMap()
{
	...
	for (int i = 0; i < Rowrender; i++)
	{
		...
		for (int a = 0; a < (int)aline.size(); a++)
		{
			...
			{
				aline[a] = (char)178;
				g_Console.writeToBuffer(a, i + 1, aline[a], 0xee);
			}
			if (aline[a] == 'b')
			{
				if (bridge == true)
			...
			if (aline[a] == 'g')
			{
				...
				{
					aline[a] = (char)178;
					g_Console.writeToBuffer(a, i + 1, aline[a], 0xbb);
				}
				else
				{
					aline[a] = (char)178;
...
void gameplay()            // gameplay logic
{
	...
	case S_STAGETWO: Stageone();
		break;
	case S_BOSSTWO: Bossone();
		break;
	case S_STAGETHREE: Stageone();
		break;
	}
}
...
void Stageone()
{
	...
	if ((g_eGamemode == S_STAGEONE) || (g_eGamemode == S_STAGETHREE))
	{
