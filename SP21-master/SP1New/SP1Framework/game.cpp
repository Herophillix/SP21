...
void init(void)
{
	...
	ifstream wordspawner("wordspawner.txt");
	string word;
	while (getline(wordspawner,word))
	{
	...
	ifstream bossmapTwo("bossmap2.txt");
	if (bossmapTwo.is_open())
	{
		for (int i = 0; i < MAP2_ROWS; i++)
		{
			Bossmap2[i] = new char[MAP2_COLUMNS];
			getline(bossmapTwo, bosstwoline);
			for (int a = 0; a < MAP2_COLUMNS; a++)
			{
				if ((bosstwoline[a] == 43) || (bosstwoline[a] == 124) || (bosstwoline[a] == 45))
				{
					bosstwoline[a] = (char)219;
				}
				Bossmap2[i][a] = bosstwoline[a];
			}
		}
	}
...
void renderBossmap()
{
	COORD c;
	string abossline;
	int BossColumns, Bossrows;
	switch (g_eGamemode)
	{
	case S_BOSSONE:
	{
		abossline.resize(MAP_COLUMNS, ' ');
		BossColumns = MAP_COLUMNS;
		Bossrows = MAP_ROWS;
		for (int i = 0; i < Bossrows; i++)
		{
			for (int a = 0; a < BossColumns; a++)
			{
	...
	case S_BOSSTWO:
	{
		abossline.resize(MAP2_COLUMNS, ' ');
		BossColumns = MAP2_COLUMNS;
		Bossrows = MAP2_ROWS;
		for (int i = 0; i < Bossrows; i++)
		{
			for (int a = 0; a < BossColumns; a++)
			{
   				abossline[a] = BaseMaze[i][a];
				switch (abossline[a])
				{
				case 'x':
				{
...
void renderInfo()
{
	...
	for (int i = 0; i < MAP_ROWS; i++)
	{
		...
		case S_STAGETWO: c.X = MAP2_COLUMNS + 2;
			break;
		case S_BOSSTWO: c.X = MAP2_COLUMNS + 2;
		}
...
void renderWords()
{
	...
	C.X = 48;
	C.Y = 15;
	g_Console.writeToBuffer(C, currentword, 0x01f);

}
void wordRandom()
{
	currentword = wordList[rand() % 35];
}
void addletterstostring()
{
	...
	{
		wordcompare.resize(wordcompare.size() -	1);
		bSomethingHappened = true;
	}
	...
	for (int convertupper = 0; convertupper < (int)currentword.size(); convertupper++)
	{
		currentword[convertupper] = toupper(currentword[convertupper]);
	}
	if (wordcompare == currentword)
	{
		wordgame++;
		Player.Health++;
		Player.Points + 10;
...
void renderwordcompare()
{
	...
	g_Console.writeToBuffer(W,wordcompare, 0x0f);
}
