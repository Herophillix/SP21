	...
	char **Maze, double &g_eBounceTime, double &g_eElapsedTime)
{
	...
	if ((bulletcheck(' ', Maze, g_sChar)) || 
		(bulletcheck((char)219, Maze, g_sChar)) || 
		(bulletcheck('w', Maze, g_sChar)) || 
		(bulletcheck('.', Maze, g_sChar)) ||
		(bulletcheck('g', Maze, g_sChar)) || 
		(bulletcheck('m', Maze, g_sChar)))
	{
...
void movebulletPRed(SGameChar &g_bulletP, SGameChar &g_portalEntrance, SGameChar &g_sChar, char **Maze)
{
	if (BulletposPRed == true)
	{
		...
		case 1:
		{
			...
			else if ((bridge == false) && (upcheck(g_bulletP, Maze, 'g')))
			{
				g_bulletP.m_cLocation.Y--;
			}
		...
		case 2:
		{
			...
			else if ((bridge == false) && (rightcheck(g_bulletP, Maze, 'g')))
			{
				g_bulletP.m_cLocation.X++;
			}
		...
		case 3:
		{
			...
			else if ((bridge == false) && (downcheck(g_bulletP, Maze, 'g')))
			{
				g_bulletP.m_cLocation.Y++;
			}
		...
		case 4:
		{
			...
			else if ((bridge == false) && (leftcheck(g_bulletP, Maze, 'g')))
			{
				g_bulletP.m_cLocation.X--;
			}
			else if (Maze[g_bulletP.m_cLocation.Y - 1][g_bulletP.m_cLocation.X - 1] != (char)219)
